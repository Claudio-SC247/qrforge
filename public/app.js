...
function applyTheme(t) {
  ...
  document.getElementById('theme-color-meta').content = t === 'dark' ? '#060610' : '#f4f4ff';
  localStorage.setItem('qreafy-theme', t);
}
function toggleMode() { applyTheme(html.getAttribute('data-theme') === 'dark' ? 'light' : 'dark'); }
(function() {
...
function onLogo(inp) {
  ...
  if (f.size > 3*1024*1024) { toast('Máx. 3 MB','er'); inp.value=''; return; }
  if (!['image/png','image/jpeg','image/gif','image/webp'].includes(f.type)) {
    toast('Formato no permitido','er'); inp.value=''; return;
  }
...
dz.addEventListener('drop', e=>{
  ...
  if(f){document.getElementById('logo-file').files=e.dataTransfer.files; onLogo({files:e.dataTransfer.files});}
});
...
async function cpQR() {
  if(!lastQR) return;
  try {
    const blob=await(await fetch(lastQR)).blob();
    await navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);
    toast('Imagen copiada','ok');
...
async function shortenURL() {
  ...
  try {
    ...
    // Save to localStorage immediately (works even if KV is down)
...
/* ══ HISTORY TOKEN ══════════════════════════════ */
//* ══ HISTORY — Sistema híbrido localStorage + Redis KV ══════ */
// localStorage: historial por dispositivo (inmediato, sin servidor)
// Redis KV:     historial persistente del servidor (sincroniza entre dispositivos)
// Estrategia: mostrar localStorage inmediatamente, luego fusionar con KV

const LS_KEY = 'qreafy:history';
const LS_MAX = 100;
...
function lsSave(items) {
  try { localStorage.setItem(LS_KEY, JSON.stringify(items.slice(0, LS_MAX))); }
  catch {}
}

function lsPush(item) {
  const items = lsLoad();
  // Avoid duplicates by short_url
  const filtered = items.filter(i => i.short_url !== item.short_url);
  lsSave([item, ...filtered]);
}
...
async function loadHistory() {
  // 1. Show localStorage immediately (zero latency)
  const local = lsLoad();
  if (local.length) renderHist(local, false);

  // 2. Try to fetch from server (Redis KV) and merge
  try {
    ...
    if (server.length) {
      // Merge: server items first (most recent), deduplicate by short_url
      const seen = new Set();
      const merged = [...server, ...local].filter(i => {
        if (seen.has(i.short_url)) return false;
        seen.add(i.short_url);
        return true;
      });
      lsSave(merged);
      renderHist(merged, true);
    } else if (local.length) {
      renderHist(local, false);
    } else {
      renderHist([], false);
    }
  } catch {
    // Server unavailable — localStorage already shown, nothing to do
  }
}

async function clearHistory() {
  // Always clear localStorage first (immediate feedback)
  lsClear();
  renderHist([]);
  toast('Historial limpiado', 'ok');

  // Then try to clear server KV (best-effort, no token needed for local clear)
  try {
    const res = await fetch('/api/history', { method: 'DELETE' });
    if (!res.ok && res.status !== 401) {
      // Silent — local already cleared, KV clear is optional
    }
  } catch {}
}
...
function renderHist(items, fromServer) {
  ...
  if(!items.length){
    ...
    li.id='hist-empty';
    li.style.cssText='color:var(--muted);font-family:var(--mono);font-size:.73rem;text-align:center;padding:1.5rem 0;';
    li.textContent='Tus URLs acortadas aparecerán aquí';
    ul.appendChild(li);
    count.textContent=''; statsBar.style.display='none'; return;
  }

  count.textContent=`${items.length} enlace${items.length!==1?'s':''}`;
  statsBar.style.display='flex';
  document.getElementById('stat-total').textContent=items.length;
  // Count items from today using ts field
  ...
  items.forEach(h=>{
    ...
    const b=document.createElement('button'); b.className='cp-btn h-cp btn-sm';
    b.textContent='⎘'; b.setAttribute('aria-label','Copiar');
    b.onclick=()=>cpText(h.short_url,b);
...
function urlToQR() {
  ...
  go('qr',document.querySelectorAll('.tab')[0]);
